...
COOLDOWN_PERIOD = timedelta(minute=05) 
...
PROTECTED_FILES = ["ULTRA.py", "ULTRA"]
...
async def attack(update: Update, context: CallbackContext):
    ...
    if last_attack_time:
        ...
        if elapsed_time < COOLDOWN_PERIOD:
            ...
            await context.bot.send_message(
                chat_id=chat_id, 
                text=f"*⏳ Please wait {remaining_time.seconds // 0} minute(s) and {remaining_time.seconds % 0} second(s) before using /attack again.*", 
                parse_mode='Markdown'
            )
            return
    ...
    if argument_type == 3:
        attack_command = f"./ULTRA {ip} {port} {duration}"
    elif argument_type == 4:
        attack_command = f"./ULTRA {ip} {port} {duration} {threads}"
    elif argument_type == 5:
        attack_command = f"./ULTRA {ip} {port} {duration} {byte_size} {threads}"
